...
<em>¿Le gustaría coordinar una reunión con el equipo de inclusión?</em>${btn}`;
		...
		// DROGAS
		case 'DROGAS':
			if (esFueraDeColegio) return `${bannerEmpatico}Fuera del liceo, las normas disciplinarias del RICE no aplican como castigo — el enfoque acá también es 100% de salud y apoyo, nunca de sancionar a tu amigo/a.<br><br>Esto es lo que puedes hacer: 1. <strong>Cuéntaselo a la Dupla Psicosocial</strong> (usa el botón de abajo) o a un adulto de confianza. 2. Ellos pueden orientarte en privado sobre cómo acercarlo a ayuda real (SENDA Previene, CESFAM), sin que tengas que resolverlo solo/a. 3. Tu identidad queda protegida siempre. ${cita('Protocolo N° 6 — Apoyo de Salud y Prevención', '')}${btn}`;
			if (es) return `${bannerEmpatico}El liceo <strong>no busca sancionar primero, sino apoyar la salud del estudiante</strong>.<br><br>Esto es lo que va a pasar: 1. Si ocurrió dentro del liceo, se registra como Falta Gravísima (Art. 41/45), pero eso no significa expulsión automática. 2. Van a citarlo/a a usted en los próximos días para conversar el plan de apoyo. 3. Si es la primera vez, se le pedirá acreditar la incorporación a tratamiento (SENDA o CESFAM) dentro de 30 días para mantener la matrícula. 4. Puede solicitar que la Dupla Psicosocial atienda a su hijo/a de forma reservada. ${cita('Protocolo N° 6 y Art. 41/45', '')}${btn}`;
			if (p.esTercero) return `${bannerEmpatico}Qué bueno que te preocupes por tu compañero/a — así se cuida a toda la comunidad, y tu identidad queda <strong>100% protegida</strong> al avisar.<br><br>Esto es lo que puedes hacer: 1. Cuéntaselo a tu Profesor Jefe, a Inspectoría, o usa el botón de abajo para hablar directo con la Dupla Psicosocial. 2. El liceo se va a acercar a tu compañero/a para ofrecerle ayuda, no para expulsarlo de entrada — el objetivo es de salud, no de castigo. ${cita('Protocolo N° 6', '')}${btn}`;
			return `${bannerEmpatico}Que hayas venido a preguntar dice mucho de ti — acá no vienes a que te castiguen.<br><br>Esto es lo que va a pasar, paso a paso: 1. <strong>Cuéntaselo a tu Profesor Jefe, a Convivencia Educativa, o usa el botón de abajo</strong> para hablar en privado con la Dupla Psicosocial ahora mismo. 2. Nadie te va a interrogar como si fueras culpable — el liceo primero se preocupa de tu salud, no te acusa. 3. Si es la primera vez, lo que sigue es apoyo (SENDA Previene / CESFAM) y acompañamiento, no una expulsión. 4. Tu apoderado va a ser citado, porque el liceo necesita que también te acompañen en esto desde la casa.<br><br><strong>¿Quieres contarme un poco de lo que está pasando?</strong> ${cita('Protocolo N° 6 — Apoyo Socioemocional', '')}${btn}`;
		...
		// ROBO
		case 'ROBO':
			if (es) return `👨‍👩‍👧‍👦 <strong>Hola, ${nombre}.</strong> Vamos a ver esto con calma.<br><br>1. La sustracción se tipifica como <strong>Falta Grave o Gravísima</strong> según haya habido fuerza o violencia. 2. Inspectoría va a entrevistar a los involucrados por separado y de forma reservada — está prohibido revisar mochilas de forma masiva o sin sospecha fundada. 3. Van a citarlo/a a usted dentro de las próximas 48 horas. 4. Si se identifica a quien lo hizo, deberá devolver lo sustraído o compensar su valor. ${cita('Protocolo N° 15 y Art. 40/44-41/45', '')}${btn}`;
			return `👋 <strong>Hola, ${nombre}.</strong> Vamos a ver esto juntos.<br><br>1. <strong>Cuéntale a tu Profesor Jefe o ve directo a Inspectoría General</strong> para que quede registrado. 2. Te van a entrevistar en privado, sin exponerte frente a tus compañeros ni hacer un "show" del tema. 3. Nadie te va a revisar la mochila a ti ni a otros sin una razón fundada — eso está prohibido. 4. Si encuentran a quien lo hizo, tiene que devolverlo o compensarlo.<br><br><strong>¿Sabes más o menos cuándo pasó?</strong> ${cita('Protocolo N° 15 — Robo y Hurto', '')}${btn}`;
...
💬 <em>¿Te gustaría que la Dupla Psicosocial te atienda hoy mismo?</em> ${cita('Ley N° 21.430 y Ley N° 20.066', '')}${btn}`;
		...
		// EMBARAZO
		case 'EMBARAZO':
			if (es) return `👨‍👩‍👧‍👦 <strong>Hola, ${nombre}.</strong> El lugar de su hijo/a en el liceo está 100% garantizado — nadie puede sugerir su retiro ni condicionar la matrícula por esto.<br><br>1. Se les va a citar a una reunión privada dentro de 48 horas para armar un plan de acompañamiento. 2. Ese plan incluye calendario de evaluaciones flexible y justificación automática de inasistencias por controles médicos. 3. Si necesita amamantar en el liceo, hay un espacio privado habilitado. 4. Se le puede eximir del uniforme y de esfuerzo físico en Educación Física. ${cita('Protocolo N° 9 — Embarazo y Maternidad/Paternidad', '')}${btn}`;
			return `👋 <strong>Hola, ${nombre}.</strong> Primero que todo: tu lugar en el liceo está 100% garantizado, nadie te puede pedir que te retires ni condicionar tu matrícula por esto.<br><br>Esto es lo que sigue: 1. <strong>Cuéntaselo a tu Profesor Jefe o a Convivencia Educativa</strong> cuando te sientas list@ — nadie más se entera sin tu autorización. 2. En un par de días te van a citar (a ti y a tu apoderado) para armar un plan de acompañamiento: evaluaciones más flexibles y permisos para tus controles médicos. 3. Si necesitas amamantar, hay un espacio privado en el liceo. 4. Puedes pedir no usar el uniforme si te complica.<br><br><strong>¿Quieres que te ayude a pensar cómo contarlo?</strong> ${cita('Protocolo N° 9 — Embarazo y Maternidad/Paternidad', '')}${btn}`;
